...
function createKanbanSource() {
  ...
  const load = () => {
    fetch("/team-ai/kanban.json", { cache: "no-store" }).then((res) => res.ok ? res.json() : Promise.reject(new Error(`HTTP ${res.status}`))).then((data) => {
      snapshot = data;
      emit();
    }).catch((error) => {
    ...
    /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(
      "button",
      {
        ...
        children: [
          ...
          !wide && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "\u56E2\u961F\u770B\u677F" })
        ]
      }
    ),
