...
/** 解析看板仓库路径：config.kanbanRepo > KANBAN_REPO 环境变量 > 未配置。 */
function resolveKanbanRepo(config) {
  const repo = config?.kanbanRepo || process.env.KANBAN_REPO;
  return repo ? resolve(repo) : null;
}

/** 解析看板状态文件路径（<repo>/docs/team/kanban.json）。 */
function resolveKanbanFile(config) {
  const repo = resolveKanbanRepo(config);
  return repo ? stateFile(repo) : null;
}
...
function handleKanbanJson(config) {
  return (req, res) => {
    ...
    const file = resolveKanbanFile(config);
    if (!file) {
      respond(
        404,
    ...
    try {
      ...
      const repo = resolveKanbanRepo(config);
      const repoName = repo ? basename(repo) : null;
